...
def count_occurrences_in_text(word, text):
    ...
    while text[i] == text[-1 - i] and (text[i] == "'" or text[i] == '_'):
        i += 1
    # # count if word is in between separators
    ps = False

    # re.split(word.lower(), text[i:-i].lower() if i > 0 else text.lower())
    for s in re.split(word.lower(), text[i:-i].lower() if i > 0 else text.lower()):
        if ps == True and (not s or s[0] in " ,.!?()«»:\"\n\u201c\u201d"):
            wc += 1
        if not s or s[-1] in " ,.!?()«»:\"\n\u201c\u201d":
            ps = True
        else:
