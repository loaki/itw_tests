...
def count_occurrences_in_subtext(word_count, up_word, low_word, len_word, text):
    wc = 0
    j=0
    prev_c = None
    for c in text:
        if j == len_word:
            if c in " ,.!?()«»:\"\n\u201c\u201d":
                wc += 1
            j = 0
        if (up_word[j] == c or low_word[j] == c) and (j > 0 or prev_c == None or prev_c in " ,.!?()«»:\"\n\u201c\u201d"):
            j += 1
        else:
            j = 0
        prev_c = c
...
def count_occurrences_in_text(word, text):
    ...
    # text markdown 
    while text[i] == text[len_text -1 - i] and (text[i] == "'" or text[i] == '_'):
        i += 1
    ...
    jobs = []
    while len_text - i > 0:
        if start + n_chunk >= end:
            p = Process(target=count_occurrences_in_subtext, args=(word_count, up_word, low_word, len_word, text[start: end]))
            jobs.append(p)
            p.start()
            break
        else:
            index_chunk = start + n_chunk
            while index_chunk < end and not text[index_chunk] in " ,.!?()«»:\"\n\u201c\u201d":
                index_chunk+=1
            p = Process(target=count_occurrences_in_subtext, args=(word_count, up_word, low_word, len_word, text[start: index_chunk]))
            start = index_chunk+1
            jobs.append(p)
            p.start()
    for proc in jobs:
